...
async def scheduledUpdateSender(_):
    ...
    print("Hello from scheduledUpdateSender!")

    addressesToBeCleared = []
    headers              = {'content-type':'application/json'}
    missDict             = {}

    if(len(scheduledUpdateDict) != 0):

        for x in scheduledUpdateDict:

            localVC = vectorClock.returnVC()

            url     = "http://" + str(x) + "/receiveScheduledUpdate"
            missDict.update({'misses':scheduledUpdateDict[x], 'vectorclock':localVC})
            payload = json.dumps(missDict)

            try:
                response = requests.put(url, headers=headers, data=payload, timeout=1)
                if(response.status_code == 200):
                    addressesToBeCleared.append(str(x))

            except(requests.exceptions.ConnectionError, requests.exceptions.ConnectTimeout, requests.exceptions.ReadTimeout) as e:
                print("From scheduled update: Server " + str(x) + " still down.")
        
        for x in range(len(addressesToBeCleared)):

            addrToBeCleared = addressesToBeCleared[x]
            del scheduledUpdateDict[addrToBeCleared]
...
class receiveScheduledUpdate(HTTPMethodView):

    async def put(self, request):

        print("Hello from receiveScheduledUpdate PUT case!")

        localVC               = vectorClock.returnVC()
        localAddress          = str(os.environ['SOCKET_ADDRESS'])
        missDict              = request.json['misses']
        incomingVC            = request.json['vectorclock']
        localId               = identifierNumDict[localAddress]
        
        if(vectorClock.VcComparator(localVC, incomingVC) == "<"):
            vectorClock.replaceVC(incomingVC)
        if(vectorClock.VcComparator(localVC, incomingVC) == "||"):
        ...
        for x in missDict:
            ...
            print("This is 'misses' from receiveScheduledUpdate PUT case: " + str(missDict))

            incomingKeyData    = missDict[x]
            incomingKeyValue   = incomingKeyData['value']
            incomingKeyVersion = incomingKeyData['version']
            incomingKeyId      = incomingKeyData['ID']

            if( x in data ):

                localKeyData    = data[x]
                localKeyValue   = localKeyData['value']
                localKeyVersion = localKeyData['version']
                localKeyId      = localKeyData['ID']
                #data.update({str(key):{'value':localValue, 'version':localVersion, 'ID':localKeyId}})

                if(localKeyVersion < incomingKeyVersion):
                    data.update({x:{'value':incomingKeyValue, 'version':incomingKeyVersion, 'ID':incomingKeyId}})
                if(localKeyVersion == incomingKeyVersion):
                    if(incomingKeyId > localKeyId):
                        data.update({x:{'value':incomingKeyValue, 'version':incomingKeyVersion, 'ID':incomingKeyId}})
        ...
        return response.json({"message":"Scheduled update handled"}, status=200)

                



        #Here is where you put the rest of the logic.  You need to iterate through 'updateDictFromRequest' and compare the version
        #numbers of each key you find there with their corresponding version numbers in the local store.  You of course might find
        #that the key in the updateDictFromRequest doesn't even exist in the local store.  In that case, just keep the key, value and version
        #number from updateDictFromRequest and update them into the local store.  You'll need tie breaking in case version numbers match.
